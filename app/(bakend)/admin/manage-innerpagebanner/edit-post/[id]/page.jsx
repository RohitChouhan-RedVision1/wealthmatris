...
export function InputForm({ postId }) {
    ...
    useEffect(() => {
        if (postId) {
            axios.get(`/api/innerpagebanner/${postId}`)
                .then(response => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            let response;
            response = await axios.put(`/api/innerpagebanner/${postId}`, formData, {
                headers: {
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 rounded px-7">
                ...
                <Button className="text-white bg-[var(--rv-primary)]" type="submit">{!loading ? 'Submit' : 'Loading...'}</Button>
            </form>
        </Form>
    );
}
        ...
        <DefaultLayout>
            <div className="flex justify-between">
                ...
                <Link href="/admin/manage-innerpagebanner/manage">
                    <Button className="text-white bg-[var(--rv-primary)]">All Advertisement</Button>
                </Link>
            </div>
