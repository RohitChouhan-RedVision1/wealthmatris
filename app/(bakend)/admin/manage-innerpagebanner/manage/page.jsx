...
const DataTableDemo = () => {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await axios.get("/api/innerpagebanner/");
                if (res.status === 200) {
                    setData(res.data);
                }
            } catch (error) {
    ...
    const handleDelete = async (id) => {
        try {
            const res = await axios.delete(`/api/innerpagebanner/${id}`);
            if (res.status === 200) {
                ...
                console.error("Failed to delete inner page banner");
            }
        } catch (error) {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent className="bg-[var(--rv-primary)] text-white" align="end">
                            <DropdownMenuLabel>Actions</DropdownMenuLabel>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={() => router.push(`/admin/manage-innerpagebanner/edit-post/${blog._id}`)}>
                                Edit
                            </DropdownMenuItem>
        ...
        <DefaultLayout>
            <div className="flex justify-between">
                ...
                <Link href="/admin/manage-innerpagebanner/add-innerpagebanner">
                    <Button className="text-white bg-[var(--rv-primary)]">Add New</Button>
                </Link>
            </div>
