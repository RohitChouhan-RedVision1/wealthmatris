...
const TableThree = ({ packageData, onDelete, allamcslogodata }) => {
  const handleDelete = async (id) => {
    try {
      if (allamcslogodata.filter((item) => item.logocategory == id).length !== 0) {
    ...
    <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
            {packageData && packageData?.map((packageItem, index) => (
              <tr key={index}>
                <td
                ...
                >
                  <p className="text-dark dark:text-white">
                    {formateDate(packageItem.createdAt)}
                  </p>
                </td> */}
                {/* <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 ${index === packageData.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <p className="text-dark dark:text-white">
                    {formateDate(packageItem.updatedAt)}
                  </p>
                </td> */}
                <td
                ...
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    <button className="hover:text-primary" onClick={() => handleDelete(packageItem._id)}>
                      <svg
